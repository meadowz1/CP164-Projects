...
class Sorted_List:
    ...
    def __getitem__(self, i):
        ...
        -------------------------------------------------------
        """
        assert 0 <= i < len(self), "Invalid index value"
    ...
    def clean(self):
        ...
        while current is not None:
            if current._value in unique_values:
                # Remove duplicate node
                if current._next is None:
                    # Update _rear if removing last node
                    self._rear = previous
                previous._next = current._next
                current = current._next
                self._count -= 1  # Decrement count for removed duplicate
            else:
